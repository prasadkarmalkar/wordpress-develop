...
function _wp_theme_json_webfonts_handler() {
	...
	$fn_validate_webfont = static function( $webfont ) {
		...
		if ( empty( $webfont['font-family'] ) || ! is_string( $webfont['font-family'] ) ) {
			wp_trigger_error( '', __( 'Webfont font family must be a non-empty string.' ) );
		...
		if ( empty( $webfont['src'] ) || ( ! is_string( $webfont['src'] ) && ! is_array( $webfont['src'] ) ) ) {
			wp_trigger_error( '', __( 'Webfont src must be a non-empty string or an array of strings.' ) );
		...
		foreach ( (array) $webfont['src'] as $src ) {
			if ( ! is_string( $src ) || '' === trim( $src ) ) {
				wp_trigger_error( '', __( 'Each webfont src must be a non-empty string.' ) );
		...
		if ( ! is_string( $webfont['font-weight'] ) && ! is_int( $webfont['font-weight'] ) ) {
			wp_trigger_error( '', __( 'Webfont font weight must be a properly formatted string or integer.' ) );
